...
import { AlertTriangle, ArrowUpDown, Building2, ChevronDown, ChevronRight, Eye, } from "lucide-react";
import { collection, doc, getDoc, getDocs, query, where, } from "firebase/firestore";
import { db, } from "../../firebase/firebase";
import { Card, StatusBadge, Table, EmptyCell, SearchInput, Select, } from "../ui/interface";
import { Button, } from "../ui/Button";
import OperatorDetail from "./OperatorDetail";
import { getCompanyById, getCompanyByNormalizedName, } from "../../lib/companies";
import { calculateOnTimeCompliance, calculateSubmissionCompletion, calculateSubmissionMetrics, calculateWorkforcePercentages, } from "../../lib/calculation-metrics";
...
const getOrganizationCategory = (organization) => {
    return normalizeValue(organization?.organizationCategory ||
        organization?.category ||
        organization?.orgType);
};
const getOrganizationLevel = (organization) => {
    return normalizeValue(organization?.type ||
 ...
 */
const isEnterpriseOperator = (organization) => {
    if (!isCompany(organization)) {
        return false;
    }
    const organizationId = getOrganizationId(organization);
    const rootEnterpriseId = organization
        ?.rootEnterpriseId;
    return (getOrganizationLevel(organization) === "enterprise" ||
 ...
 */
const isOrganizationOrDescendant = (organization, parentOrganizationId) => {
    ...
    return (organizationId ===
        ...
        organization?.rootEnterpriseId ===
            parentOrganizationId ||
        ancestorIds.includes(parentOrganizationId));
};
/*
...
const getReportDate = (report) => {
    ...
    return (toDate(report.reportingDate) ||
...
const getActualSubmittedAt = (report) => {
    return (toDate(report?.submittedAt) ||
...
const getSubmittedAt = (report) => {
    return (getActualSubmittedAt(report) ||
        toDate(report?.updatedAt) ||
        getReportDate(report));
};
const getDeadlineAt = (report) => {
    return (toDate(report?.deadlineAt) ||
        toDate(report?.dueAt) ||
...
const isReportSubmittedLate = (report) => {
    ...
    if (normalizeStatus(report?.status) ===
        "submitted_late") {
        return true;
    }
...
const isReportEligibleForCompliance = (report, now = new Date()) => {
    ...
    const deadline = getDeadlineAt(report);
    const deadlineHasPassed = Boolean(deadline &&
        deadline <=
            now);
    return (isReportSubmitted(report) ||
        deadlineHasPassed ||
...
const getReportFields = (report) => {
    ...
    return (report.formSnapshot?.fields ||
...
/*
 * The first submitted workflow event is the original report submitter.
 * Later approvals must not replace that person in reporting history.
 */
const getOriginalSubmitter = (report) => {
    const history = Array.isArray(report?.workflowHistory)
        ? report.workflowHistory
        : [];
    const submission = history.find((event) => event?.action === "submitted" &&
        event?.role !== "system") || null;
    return {
...
const getWorkforceUpdatedAt = (record) => {
    return (toDate(record?.updatedAt) ||
 ...
 */
const isWorkforceRecordInOrganizationScope = ({ record, organization, organizationMap, }) => {
    const selectedOrganizationId = getOrganizationId(organization);
    if (!selectedOrganizationId) {
        return false;
    }
    const selectedLevel = getOrganizationLevel(organization);
    ...
    const recordOrganization = recordOrganizationId
        ? organizationMap.get(recordOrganizationId)
        : null;
    const recordAncestorIds = Array.from(new Set([
        ...(Array.isArray(record?.ancestorIds)
            ? record.ancestorIds
            : []),
        ...(Array.isArray(recordOrganization
            ?.ancestorIds)
            ? recordOrganization
                .ancestorIds
            : []),
    ]));
    if (recordOrganization?.parentId ===
        selectedOrganizationId ||
        recordAncestorIds.includes(selectedOrganizationId)) {
        return true;
    }
    const selectedRootEnterpriseId = organization.rootEnterpriseId ||
        (selectedLevel ===
            "enterprise"
            ? selectedOrganizationId
            : "");
    const recordRootEnterpriseId = record.rootEnterpriseId ||
        record.enterpriseId ||
        recordOrganization
            ?.rootEnterpriseId ||
        (getOrganizationLevel(recordOrganization) === "enterprise"
            ? getOrganizationId(recordOrganization)
            : "");
    if (selectedLevel ===
        "enterprise") {
        return (recordRootEnterpriseId ===
            selectedOrganizationId);
    }
    if (selectedLevel ===
        "region") {
        const selectedRegionId = normalizeRegionId(organization.regionId);
        const recordRegionId = normalizeRegionId(record.regionId ||
            recordOrganization
                ?.regionId);
        /*
         ...
         */
        return Boolean(selectedRegionId &&
...
const getOrganizationLogo = (organization) => {
    const companyById = getCompanyById(organization?.companyId);
    if (companyById?.logo) {
    ...
    return (getCompanyByNormalizedName(normalizedName)?.logo ||
        "");
};
...
const CompanyLogo = ({ name, logoUrl, }) => {
    const initials = String(name ||
        ...
        .slice(0, 2)
        .toUpperCase();
    return (<div className="flex h-10 w-10 shrink-0 items-center justify-center overflow-hidden rounded-lg border border-slate-200 bg-white">
      {logoUrl ? (<img src={logoUrl} alt={`${name} logo`} className="h-full w-full object-contain p-1"/>) : (<span className="text-xs font-semibold text-navy-700">
          {initials}
        </span>)}
    </div>);
};
const SortHeader = ({ label, sortKey, activeSortKey, sortDirection, onSort, }) => {
    const isActive = activeSortKey ===
        sortKey;
    return (<th className="cursor-pointer select-none whitespace-nowrap px-4 py-3 text-left text-xs font-medium text-slate-500 hover:text-navy-700" onClick={() => onSort(sortKey)}>
      <span className="inline-flex items-center gap-1">
        {label}

        <ArrowUpDown className={`h-3 w-3 ${isActive
            ? "opacity-100"
            : "opacity-40"} ${isActive &&
            sortDirection ===
                "desc"
...
const loadScopedOrganizations = async (organizationId) => {
    const currentOrganizationSnapshot = await getDoc(doc(db, ORGANIZATIONS_COLLECTION, organizationId));
    if (!currentOrganizationSnapshot.exists()) {
        throw new Error("The user's organization could not be found.");
    }
    ...
    if (isMinistry(currentOrganization)) {
        ...
        const snapshot = await getDocs(query(collection(db, ORGANIZATIONS_COLLECTION), where("sector", "==", sector)));
        scopedOrganizations =
            snapshotDocuments(snapshot);
    }
    else if (organizationLevel ===
        "enterprise") {
        const snapshot = await getDocs(query(collection(db, ORGANIZATIONS_COLLECTION), where("rootEnterpriseId", "==", organizationId)));
        scopedOrganizations =
            snapshotDocuments(snapshot);
    }
    else if (organizationLevel ===
    ...
    return {
        currentOrganization,
        organizations: mergeDocumentLists([
            scopedOrganizations,
            [
                currentOrganization,
            ],
...
const loadFuelPricesForOrganizations = async (organizations) => {
    const enterpriseIds = Array.from(new Set(organizations
        .map((organization) => {
        const organizationId = getOrganizationId(organization);
        ...
        .filter(Boolean)));
    const snapshots = await Promise.all(enterpriseIds.map((enterpriseId) => getDoc(doc(db, COMPANY_FUEL_PRICES_COLLECTION, enterpriseId))));
    return snapshots
...
const OperatorsTab = ({ currentUser = null, complianceThreshold = null, onSelectOperator = () => { }, }) => {
    ...
    const [organizationsLoadedAt, setOrganizationsLoadedAt,] = useState(null);
    ...
    useEffect(() => {
        ...
        const loadOperatorData = async () => {
            try {
                ...
                const memberSnapshot = await getDoc(doc(db, ORGANIZATION_MEMBERS_COLLECTION, currentUser.uid));
                if (!memberSnapshot.exists()) {
                    throw new Error("This user's organization membership could not be found.");
                }
                const organizationId = memberSnapshot.data()
                    ?.organizationId ||
                    "";
                if (!organizationId) {
                    throw new Error("This user is not linked to an organization.");
                }
                ...
                const signedInLevel = getOrganizationLevel(signedInOrganization);
                const belongsToOrganization = (organization, parentOrganizationId) => {
                    const ancestorIds = Array.isArray(organization?.ancestorIds)
                        ? organization.ancestorIds
                        : [];
                ...
                const sortOrganizationsByName = (organizationList) => {
                    return [
                        ...organizationList,
                ...
                if (isMinistry(signedInOrganization)) {
                    /*
                     * Ministry: show enterprise operators only.
                     */
                    matchingCompanies =
                        sortOrganizationsByName(organizations.filter(isEnterpriseOperator));
                }
                else if (isCompany(signedInOrganization)) {
                    if (signedInLevel ===
                        "enterprise") {
                        /*
                         ...
                         */
                        matchingCompanies =
                            sortOrganizationsByName(organizations.filter((organization) => getOrganizationLevel(organization) ===
                                "region" &&
                                belongsToOrganization(organization, signedInOrganizationId)));
                    }
                    else if (signedInLevel ===
                        "region") {
                        /*
                    ...
                    throw new Error("The organization category must be ministry or company.");
                }
                if (!requestIsActive) {
                    return;
                }
                ...
                setOrganizationsLoadedAt(new Date());
            }
            catch (error) {
    ...
    const organizationMap = useMemo(() => {
        return new Map(allOrganizations.map((organization) => [
    ...
    const enrichedReports = useMemo(() => {
        return reportSubmissions.map((report) => {
            const organization = organizationMap.get(report.organizationId) ||
                {};
            const enterpriseId = organization
                .rootEnterpriseId ||
                getOrganizationId(organization) ||
                report.organizationId;
            const enterprise = organizationMap.get(enterpriseId) ||
                organization;
            const priceRecord = report.pricingSnapshot ||
                priceMap.get(report.organizationId) ||
                priceMap.get(enterpriseId) ||
                {};
            const calculatedFallback = calculateSubmissionMetrics({
                fields: getReportFields(report),
                fieldValues: getReportValues(report),
                petrolPrice: priceRecord.petrolPrice ??
                    priceRecord.petrolPricePerLitre ??
                    0,
                dieselPrice: priceRecord.dieselPrice ??
            ...
            return {
                 ...
                 */
                referencePrices: {
                    petrolPrice: toNumber(priceRecord.petrolPrice ??
                        priceRecord.petrolPricePerLitre),
                    dieselPrice: toNumber(priceRecord.dieselPrice ??
                        priceRecord.dieselPricePerLitre),
                },
     ...
     */
    const buildOperatorData = (organization) => {
        const organizationId = getOrganizationId(organization);
        ...
        const submittedToday = expectedToday.filter(isReportSubmitted);
        const submittedLateToday = submittedToday.filter(isReportSubmittedLate);
        const productionToday = submittedToday.reduce((total, report) => total +
            toNumber(report.calculatedMetrics
                .total_volume_sold), 0);
        const estimatedDailyRevenue = submittedToday.reduce((total, report) => total +
            toNumber(report.calculatedMetrics
        ...
        const complianceOnTimeReports = complianceEligibleReports.filter(isReportSubmittedOnTime);
        const complianceLateReports = complianceEligibleReports.filter(isReportSubmittedLate);
        const reportsExpected = complianceEligibleReports.length;
        const reportsSubmitted = complianceSubmittedReports.length;
        const reportsSubmittedOnTime = complianceOnTimeReports.length;
        const reportsSubmittedLate = complianceLateReports.length;
         ...
         */
        const scopedWorkforceRecords = workforceRecords.filter((record) => isWorkforceRecordInOrganizationScope({
            record,
            organization,
            organizationMap,
        }));
        const workforce = scopedWorkforceRecords.reduce((totals, record) => ({
            local: totals.local +
        ...
        const workforcePercentages = calculateWorkforcePercentages({
            localEmployees: workforce.local,
            expatEmployees: workforce.expat,
        });
        const submittedScopedReports = scopedReports.filter(isReportSubmitted);
        /*
         ...
         */
        const latestSubmittedReport = [...submittedScopedReports]
            .filter((report) => toNumber(report.calculatedMetrics
            .total_volume_sold) > 0 ||
            toNumber(report.sourceMetrics
                .petrol_volume_sold) > 0 ||
            toNumber(report.sourceMetrics
                .diesel_volume_sold) > 0)
            .sort((first, second) => (getSubmittedAt(second)?.getTime() ||
            0) -
            (getSubmittedAt(first)?.getTime() ||
                0))[0] ||
            null;
        const hasTodayProduction = productionToday > 0;
        const displayedProduction = hasTodayProduction
            ? productionToday
            : toNumber(latestSubmittedReport
                ?.calculatedMetrics
                ?.total_volume_sold);
        const displayedRevenue = hasTodayProduction
            ? estimatedDailyRevenue
            : toNumber(latestSubmittedReport
                ?.calculatedMetrics
                ?.estimated_daily_revenue);
        const productionDataDate = hasTodayProduction
            ? today
            : getReportDate(latestSubmittedReport) ||
                getSubmittedAt(latestSubmittedReport);
        ...
        return {
            ...
            industrySegment: organization.industrySegment,
            rootEnterpriseId: organization.rootEnterpriseId,
            ancestorIds: organization.ancestorIds,
            parentId: organization.parentId ||
                "",
            organizationLevel: getOrganizationLevel(organization),
            region: regionLabel,
            ...
            status,
            branches,
            productionToday: displayedProduction,
            estimatedDailyRevenue: displayedRevenue,
            productionDataDate,
            ...
            /*
                   ...
                   */
            submissionsExpectedToday: expectedToday.length,
            submissionsToday: `${submittedToday.length}/${expectedToday.length}`,
            workforce: {
                local: workforce.local,
                expat: workforce.expat,
                vacancies: workforce.vacancies,
                futureNeed: workforce.futureNeed,
                shortage: workforce.shortage,
                localPercentage: workforcePercentages
                    .localWorkforcePercentage,
                expatPercentage: workforcePercentages
                    .expatWorkforcePercentage,
                total: workforcePercentages
                    .totalWorkforce,
            },
            localWorkforce: workforce.local,
            localWorkforcePct: workforcePercentages
                .localWorkforcePercentage,
            /*
            ...
            updatedAt: latestReportDate,
        };
    };
    const mergedOperators = useMemo(() => {
        return visibleOrganizations.map(buildOperatorData);
    }, [
        allOrganizations,
        enrichedReports,
        organizationsLoadedAt,
    ...
    const isBranchAccount = currentOrganizationLevel ===
        "branch";
    const isMinistryAccount = isMinistry(currentOrganization);
    const listLabel = isMinistryAccount
        ? {
            title: "Operators",
            singular: "Operator",
            plural: "operators",
            search: "Search operators or child organizations…",
        }
        : currentOrganizationLevel ===
            ...
            ? {
                ...
                singular: "Region",
                plural: "regions",
                search: "Search regions or branches…",
            }
            : {
                title: "Branches",
                singular: "Branch",
    ...
    const canExpandHierarchyRows = isMinistryAccount ||
        currentOrganizationLevel ===
            "enterprise";
    /*
     ...
     * data has loaded.
     */
    const branchDefaultOperator = isBranchAccount
        ? mergedOperators.find((operator) => getOrganizationId(operator) ===
    ...
    const regionOptions = useMemo(() => {
        return [
            ...new Set([
                ...mergedOperators.map((operator) => operator.region),
                ...mergedOperators
                    .flatMap((operator) => operator.branches)
    ...
    const filteredOperators = useMemo(() => {
        ...
        const filtered = mergedOperators.filter((operator) => {
            const matchesSearch = !normalizedSearch ||
                ...
                operator.branches.some((branch) => normalizeValue(branch.name ||
                    branch.branch).includes(normalizedSearch));
            const matchesRegion = !regionFilter ||
                operator.region ===
                    regionFilter ||
                operator.branches.some((branch) => branch.region ===
                    regionFilter);
        ...
        return [
            ...
            const firstValue = firstOperator[sortKey] ??
                "";
            const secondValue = secondOperator[sortKey] ??
                "";
            if (typeof firstValue ===
                "number" ||
                typeof secondValue ===
                    "number") {
                const comparison = (Number(firstValue) ||
                    0) -
                    (Number(secondValue) ||
                        0);
    ...
    const flaggedOperators = useMemo(() => {
        ...
        return mergedOperators.filter((operator) => {
            const compliance = Number(operator.compliance);
            return (Number.isFinite(compliance) &&
                compliance <
                    complianceThreshold);
        });
    }, [
    ...
    const dataUpdatedAt = useMemo(() => {
        return (mergedOperators
            .map((operator) => operator.updatedAt)
            .filter(Boolean)
            .sort((first, second) => (toDate(second)?.getTime() ||
            0) -
            (toDate(first)?.getTime() ||
                0))[0] ||
            organizationsLoadedAt);
    }, [
        mergedOperators,
     ...
     */
    const scopeLabel = isMinistry(currentOrganization)
    ...
    const toggleOperator = (operatorId) => {
        setExpandedOperatorId((currentOperatorId) => currentOperatorId ===
            operatorId
            ? null
            : operatorId);
    };
    const updateDetailView = (updateState) => {
        updateState();
    ...
    const handleSelectChildOrganization = (organization) => {
        updateDetailView(() => {
            if (selectedOperator) {
            ...
            setSelectedOperator(organization);
        });
        onSelectOperator?.(organization);
    };
    const handleDetailBack = () => {
        updateDetailView(() => {
            if (detailHistory.length >
                0) {
                const previousOrganization = detailHistory[detailHistory.length -
                    1];
                setDetailHistory((currentHistory) => currentHistory.slice(0, -1));
                setSelectedOperator(previousOrganization);
                return;
            }
    ...
    if (activeOperator) {
        ...
        <OperatorDetail key={activeOperator.organizationId ||
                activeOperator.id} operator={activeOperator} updatedAt={activeOperator.updatedAt ||
                organizationsLoadedAt} backLabel={detailHistory.length >
                0
                ? `Back to ${detailHistory[detailHistory.length -
                    1]?.name ||
                    "Parent Organization"}`
                : "Back to Operators"} onBack={handleDetailBack} onSelectOrganization={handleSelectChildOrganization} showBackButton={!isBranchAccount}/>
      </div>);
    }
    return (<section className="min-h-full w-full bg-slate-50 px-4 py-6 sm:px-5 lg:px-6">
      {/*
      ...
      <header className="mb-8 flex flex-col justify-between gap-4 border-b border-slate-200 pb-6 sm:flex-row sm:items-end">
        <div>
          <div className="mb-2 flex flex-wrap items-center gap-3">
            <span className="h-6 w-1.5 rounded-full" style={{
          ...
          <p className="text-sm text-slate-500">
            Monitor production, estimated revenue, reporting compliance and workforce performance across this scope.
          </p>
        </div>
      ...
      {loadError && (<div className="mb-5 rounded-lg border border-red-200 bg-red-50 px-4 py-3">
      ...
      {flaggedOperators.length >
            0 && (<div className="mb-5 flex items-start gap-3 rounded-lg border border-amber-300 bg-amber-50 px-4 py-3">
          ...
          <div className="min-w-0 flex-1">
            <p className="text-sm text-amber-800">
              <span className="font-semibold">
                ...
                {flaggedOperators.length ===
                1
                ? `${listLabel.singular.toLowerCase()} requires`
                : `${listLabel.plural} require`}{" "}
                attention
              </span>
              ...
              {flaggedOperators
                .map((operator) => `${operator.name || "Unnamed operator"} is at ${formatNumber(operator.compliance, 1)}% compliance`)
                .join(", ")}
              .
            </p>

            <button type="button" onClick={() => handleSelectOperator(flaggedOperators[0])} className="mt-1 text-sm font-medium text-amber-900 underline hover:no-underline">
              View details →
            </button>
          </div>
        </div>)}
      ...
      <div className="mb-5 flex flex-wrap items-center gap-3">
        <SearchInput value={search} onChange={setSearch} placeholder={listLabel.search}/>
        {(isMinistryAccount ||
            currentOrganizationLevel === "enterprise") && (<Select value={regionFilter} onChange={setRegionFilter} options={regionOptions} placeholder="All Regions"/>)}
      ...
      <Card className="overflow-hidden">
          ...
          <table className="w-full min-w-[1060px]">
            <thead>
              <tr className="border-b border-slate-200">
                <th className="w-10 px-4 py-3" aria-label="Expand operator"/>

                <SortHeader label={listLabel.singular} sortKey="name" activeSortKey={sortKey} sortDirection={sortDirection} onSort={toggleSort}/>

                <SortHeader label="Latest Production" sortKey="productionToday" activeSortKey={sortKey} sortDirection={sortDirection} onSort={toggleSort}/>

                <SortHeader label="Latest Estimated Revenue" sortKey="estimatedDailyRevenue" activeSortKey={sortKey} sortDirection={sortDirection} onSort={toggleSort}/>

                <SortHeader label="Local Workforce %" sortKey="localWorkforcePct" activeSortKey={sortKey} sortDirection={sortDirection} onSort={toggleSort}/>

                <th className="whitespace-nowrap px-4 py-3 text-left text-xs font-medium text-slate-500">
                  Submissions Today
                </th>

                <SortHeader label="On-time Compliance" sortKey="compliance" activeSortKey={sortKey} sortDirection={sortDirection} onSort={toggleSort}/>
            ...
            <tbody>
              {loading ? (<tr>
                ...
                </tr>) : filteredOperators.length >
            0 ? (filteredOperators.map((operator) => {
            const operatorId = operator.organizationId ||
                operator.id ||
                operator.name;
            const operatorBranches = Array.isArray(operator.branches)
                ? operator.branches
                : [];
            const canExpandOperator = canExpandHierarchyRows &&
                operatorBranches.length >
                    0;
            const isExpanded = canExpandOperator &&
                expandedOperatorId ===
                    operatorId;
            ...
            const requiresAttention = [
                ...
                "missing",
                "overdue",
            ].includes(normalizeStatus(displayStatus));
            return (<Fragment key={operatorId}>
                        <tr className={`border-b border-slate-100 text-[13px] text-navy-900 transition-colors ${requiresAttention
                    ? "bg-amber-50/40 hover:bg-amber-50/70"
                    : "cursor-pointer hover:bg-slate-50"}`} onClick={() => handleSelectOperator(operator)}>
                          <td className="px-4 py-3">
                            {canExpandOperator ? (<button type="button" onClick={(event) => {
                        event.stopPropagation();
                        toggleOperator(operatorId);
                    }} aria-expanded={isExpanded} aria-label={isExpanded
                        ? `Collapse ${operator.name}`
                        : `Expand ${operator.name}`} className="rounded p-1 transition-colors hover:bg-slate-200">
                                {isExpanded ? (<ChevronDown className="h-4 w-4 text-slate-500"/>) : (<ChevronRight className="h-4 w-4 text-slate-500"/>)}
                              </button>) : (<span className="block h-6 w-6"/>)}
                          </td>

                          <td className="px-4 py-3">
                            <div className="flex items-center gap-3">
                              <CompanyLogo name={operator.name} logoUrl={operator.logoUrl}/>
                              ...
                              <div className="min-w-0">
                                ...
                                {(operator.country ||
                    operator.industrySegment) && (<p className="mt-0.5 whitespace-nowrap text-xs text-slate-400">
                                    {[
                        operator.country,
                        operator.industrySegment,
                    ]
                    ...
                    0 ? (<div>
                                <p className="font-semibold text-navy-950">
                                  {formatNumber(operator.productionToday)}{" "}
                                  L
                                </p>

                                {operator.productionIsCarriedForward &&
                        operator.productionDataDate && (<p className="mt-0.5 text-[10px] text-slate-400">
                                      Last reported{" "}
                                      {formatDate(operator.productionDataDate)}
                                    </p>)}
                              </div>) : ("—")}
                          </td>
                    ...
                    0 ? (<div>
                                <p className="font-semibold text-navy-950">
                                  {formatCurrency(operator.estimatedDailyRevenue)}
                                </p>
                    ...
                    ?.total >
                    0
                    ? `${formatNumber(operator.localWorkforcePct, 1)}%`
                    : "—"}
                          </td>

                          <td className="whitespace-nowrap px-4 py-3">
                            <EmptyCell value={operator.submissionsExpectedToday >
                    0
                    ...
                    : "—"}
                          ...
                          <td className="px-4 py-3">
                            <Button variant="ghost" size="sm" className="text-slate-600 hover:bg-slate-100 hover:text-navy-950" onClick={(event) => {
                    event.stopPropagation();
                    handleSelectOperator(operator);
                        ...
                        {canExpandOperator &&
                    isExpanded && (<tr className="bg-slate-50/60">
                        ...
                        0 ? (<Table headers={[
                            ...
                            "Status",
                            "Details",
                        ]} rows={operatorBranches} accentKey="status" renderRow={(branch) => {
                            const childName = branch.name ||
                                branch.branch ||
                                "Unnamed organization";
                            const openChild = () => handleSelectOperator(branch);
                            return (<>
                                          <td onClick={openChild} className="cursor-pointer whitespace-nowrap px-4 py-2.5">
                                            <div className="flex items-center gap-3">
                                              <CompanyLogo name={childName} logoUrl={branch.logoUrl}/>
                                    ...
                                    "organization"}
                                          ...
                                          <td onClick={openChild} className="cursor-pointer whitespace-nowrap px-4 py-2.5 font-semibold text-navy-900">
                                          ...
                                          <td onClick={openChild} className="cursor-pointer whitespace-nowrap px-4 py-2.5 font-semibold tabular-nums text-navy-900">
                                            <EmptyCell value={branch.productionToday >
                                    0
                                    ? `${formatNumber(branch.productionToday)} L`
                                    : null}/>
                                          </td>
                                    ...
                                    : null}/>
                                          ...
                                          <td onClick={openChild} className="cursor-pointer whitespace-nowrap px-4 py-2.5 font-semibold tabular-nums text-navy-900">
                                            <EmptyCell value={branch.estimatedDailyRevenue >
                                    0
                                    ...
                                    : null}/>
                                          ...
                                          <td onClick={openChild} className="cursor-pointer px-4 py-2.5">
                  ...
                  <td colSpan={9} className="px-4 py-14 text-center">
                    ...
                    <p className="mt-3 text-sm font-medium text-slate-500">
                      No {listLabel.plural} found
                    </p>

                    <p className="mt-1 text-xs text-slate-400">
                      No {listLabel.plural} are available within your access scope.
                    </p>
                  </td>
                </tr>)}
            </tbody>
        ...
        <div className="border-t border-slate-200 px-4 py-3 text-xs text-slate-500">
          ...
          {mergedOperators.length} {listLabel.plural}
        </div>
      </Card>
    </section>);
};
