  ...
  useEffect(() => {
    const loadAccountData = async () => {
      ...
      try {
        ...
        setOrganizationLogo(company?.logo || "");
        ...
        const hierarchyWithAdmins = await Promise.all(
          validOrganizations.map(async (organizationItem, index) => {
            ...
            const hierarchyCompany = getCompanyByNormalizedName(
              validOrganizations[0]?.normalizedName
            );

            return {
              id: organizationItem.organizationId,

              level: organizationItem.type,

              name: organizationItem.name,

              parent: index > 0 ? validOrganizations[index - 1]?.name : "",

              adminName: administrator?.fullName || administrator?.email || "",

              adminRole: formatRole(administrator?.role),

              logo: hierarchyCompany?.logo || company?.logo || "",
            };
          })
        );
  ...
  return (
    <div>
      ...
      <p className="-mt-4 mb-8 text-sm text-slate-500">
        Manage your profile, team members and organization structure.
      </p>
        ...
        <div className="space-y-6">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="-mt-2 text-sm text-slate-500">
                {teamMembers.length}{" "}
                {teamMembers.length === 1 ? "member" : "members"} linked to this
                organization
              </p>
            </div>
          ...
          <Card className="overflow-hidden">
            {teamMembers.length > 0 ? (
              <Table
                headers={["Member", "Job Title", "Role", "Organization Level"]}
                rows={teamMembers}
                renderRow={(member) => {
      ...
      {activeTab === "organization" && (
        <div className="space-y-6">
          <div>
            <SectionHeader>Organization Hierarchy</SectionHeader>

            <p className="-mt-2 text-sm text-slate-500">
              Review your organizational structure from the highest level to
