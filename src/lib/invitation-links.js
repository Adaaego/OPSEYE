...
/*
 * Public invitation pages validate the token through a callable Function so
 * Firestore can remain closed to unauthenticated clients.
 */
...
const validatePublicInvitationCall =
  httpsCallable(
    functions,
 ...
 *
 * Before sign-in, the browser must not read organizationInvitations directly.
 * The callable function performs the lookup server-side and returns only the
 * invitation summary required by the signup page.
 *
 * Once Firebase Authentication exists, the full invitation is loaded directly
 * because the authenticated onboarding flow still needs fields such as
 * invitedBy and hierarchy metadata. Firestore rules will restrict that read
 * to the authenticated email that owns the invitation.
 */
export const validateInvitation = async ({
  ...
  if (!currentUser) {
    ...
    if (
      normalizedExpectedEmail &&
      normalizeEmail(
        publicInvitation.emailLower ||
        publicInvitation.email
      ) !== normalizedExpectedEmail
    ) {
      return {
        valid: false,
